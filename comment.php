...
if (strlen($_SESSION['detsuid']==0)) {
  ...
  $comment=$_POST['comment'];
     ...
     $query=mysqli_query($con, "update tbluser set FullName ='$fullname', Comment='$comment', MobileNumber='$mobno' where ID='$userid'");
    if ($query) {
    $msg="Comment Added Check Dashboard";
  }
  else
    {
...
<input type="email" class="form-control" name="email" value="<?php  echo $row['Email'];?>" required="true" readonly="true">
									...
									<div class="form-group">
									<label>Comment</label>
	<input class="form-control" type="text" placeholder="Comment Limit 60 character" value="<?php  echo $row['Comment'];?>" name="comment" required="true">
								</div>
